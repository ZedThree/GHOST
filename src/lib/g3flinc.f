...
C
C          ------------------------------------------------
C          ROUTINE NO. (3003)   VERSION (A7.2C)   21:FEB:86
C          ------------------------------------------------
C
...
C
C          (THIS VERSION IS FOR TRENC/DECODE).
C
C
...
C
    ...
    2     CALL G2FILE(NMFILE,NMPICT,INFOFO,INBUFR,
     &                LFILEN,LPICTN,LNFOFO,NPIC,3,ISTATE)
          IF (ISTATE.GT.2) GO TO 9
C
C          IF THERE IS INFORMATION ASSOCIATED WITH A PICTURE,
C          IT IS LOADED INTO THE PICTURE (OUTPUT) INFORMATION
C          ARRAY WHEN THE FIRST (PICTURE) HEADER BLOCK IS READ.
C
          LNFOPO= MOD(INBUFR(1),256)
          NWORDS= 0
          IF (LNFOPO.LE.0) GO TO 3
C
          NWORDS= ((LNFOPO-1)/NMCHI)+1
C
          DO 300 LOAD= 1,NWORDS
            INFOPO(LOAD)= INBUFR(LOAD+2)
  300     CONTINUE
C
