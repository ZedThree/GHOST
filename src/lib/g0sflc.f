      SUBROUTINE G0SFLC(SURFAS,ISTRTX,ISTOPX,NPTSX,ISTRTY,ISTOPY,
     &                  NPTSY,CLEVLS,ISTRTL,ISTOPL,MODE)
C
C          ------------------------------------------------
C          ROUTINE NO. ( 360)   VERSION (A9.1)    13:JAN:94
C          ------------------------------------------------
C
      PARAMETER (ISZARR= 1200, JSZARR= 600)
      REAL    SURFAS(NPTSX,NPTSY),CLEVLS(ISTOPL),XPOS(JSZARR),
     &        YPOS(JSZARR),RDATA(1),XADDPT(3),YADDPT(3),SXPT(4),
     &        SYPT(4),SHPT(4)
...
C
      DO 200 JJ= ISTRTB,ISTOPB-INCB,INCB
        ...
        INDR= -1
        IF (IDIR.EQ.0) THEN
          SHPT(1)= MAX(MIN(SURFAS(ISTRTA,JJ),SURMAX),SURMIN)
          SHPT(2)= MAX(MIN(SURFAS(ISTRTA,JJ+INCB),SURMAX),SURMIN)
        ELSE
          SHPT(1)= MAX(MIN(SURFAS(JJ,ISTRTA),SURMAX),SURMIN)
          SHPT(2)= MAX(MIN(SURFAS(JJ+INCB,ISTRTA),SURMAX),SURMIN)
        ENDIF
C
        SXPOS1= INDC*DELTXC+XSHIFT
        SYPOS1= INDC*DELTYC+VSCALE*SHPT(1)+YSHIFT
        SXPOS2= INDC1*DELTXC+XSHIFT
        SYPOS2= INDC1*DELTYC+VSCALE*SHPT(2)+YSHIFT
C
        DO 200 II= ISTRTA,ISTOPA-INCA,INCA
          INDR= INDR+1
          INDR1= INDR+1
          IF (IDIR.EQ.0) THEN
            SHPT(3)= MAX(MIN(SURFAS(II+INCA,JJ+INCB),SURMAX),SURMIN)
            SHPT(4)= MAX(MIN(SURFAS(II+INCA,JJ),SURMAX),SURMIN)
          ELSE
            SHPT(3)= MAX(MIN(SURFAS(JJ+INCB,II+INCA),SURMAX),SURMIN)
            SHPT(4)= MAX(MIN(SURFAS(JJ,II+INCA),SURMAX),SURMIN)
          ENDIF
C
          SXPOS3= INDR1*DELTXR+INDC1*DELTXC+XSHIFT
          SYPOS3= INDR1*DELTYR+INDC1*DELTYC+VSCALE*SHPT(3)+YSHIFT
          SXPOS4= INDR1*DELTXR+INDC*DELTXC+XSHIFT
          SYPOS4= INDR1*DELTYR+INDC*DELTYC+VSCALE*SHPT(4)+YSHIFT
C
...
C          CASE 3
C
            CALL G0SCLP(XPOS,YPOS,INDST,IND-1,XADDPT,YADDPT,INDADD,
     &                  SXPT,SYPT,SHPT,CLEVLS,ISTRTL,ISTOPL,MODE)
C
            INDADD= 0
...
C          CASE 2
C
            CALL G0SCLP(XPOS,YPOS,INDST,IND,XADDPT,YADDPT,INDADD,
     &                  SXPT,SYPT,SHPT,CLEVLS,ISTRTL,ISTOPL,MODE)
C
            INDADD= 0
          ENDIF
...
C
   13     IF (INDC.EQ.0) THEN
            ...
            CALL POSITN(SXPOS1,SYPOS1)
            CALL JOIN(SXPOS4,SYPOS4)
            IF (II.EQ.ISTOPA-INCA) CALL JOIN(SXPOS4,YBAS4)
C
          ENDIF
C
          IF (INDR.EQ.0) THEN
            ...
            CALL JOIN(SXPOS2,SYPOS2)
C
            IF (JJ.EQ.ISTOPB-INCB) CALL JOIN(SXPOS2,YBAS2)
            IF (INDC.EQ.0) THEN
              CALL POSITN(SXPOS1,SYPOS1)
              CALL JOIN(SXPOS1,YBAS1)
            ENDIF
...
C
      ...
      IF (ITPBTM.EQ.1.AND.INDAXE.EQ.0) GO TO 15
C
C          DRAW BASE LINES.
C
...
C
   ...
   15 IF (ISURIN.NE.0) CALL G0SIND(XEND2,XEND4,YEND2,YEND4)
C
      RETURN
C
  901 NUMERR= 41
