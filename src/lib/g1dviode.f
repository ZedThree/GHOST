...
C
C          ------------------------------------------------
C          ROUTINE NO. (1005)   VERSION (A8.1DE)  14:NOV:90
C          ------------------------------------------------
C
C          THIS INTERFACES DEVICE I/O WITH THE SYSTEM.
C          (THIS VERSION IS FOR GRID DECODER ).
C
C
...
C
      INTEGER   IDATA(NCHRS)
      LOGICAL   EXISTS
      CHARACTER NAMDEC*128,CHCMND*128
C
...
C
      INQUIRE(FILE=NAMDEC(1:LENDEC),EXIST=EXISTS)
      IF (.NOT.EXISTS) GO TO 1
C
      OPEN(UNIT=KDISPO,FILE=NAMDEC(1:LENDEC),STATUS='OLD')
      CLOSE(UNIT=KDISPO,STATUS='DELETE')
    1 OPEN(UNIT=KDISPO,FILE=NAMDEC(1:LENDEC),STATUS='NEW')
      RETURN
C
