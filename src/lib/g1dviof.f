...
C
C          ------------------------------------------------
C          ROUTINE NO. (1005)   VERSION (A8.1F)   03:DEC:90
C          ------------------------------------------------
C
C          THIS INTERFACES DEVICE I/O WITH THE SYSTEM.
C          (THIS VERSION IF FOR GENERAL OUTPUT TO A FILE).
C
C
...
C
      ...
      CHARACTER CHCMND*128,NAMGRA*128
C
...
C
      INQUIRE(FILE=NAMGRA(1:LENGRA),EXIST=EXISTS)
      IF (.NOT.EXISTS) GO TO 1
C
      OPEN(UNIT=KDISPO,FILE=NAMGRA(1:LENGRA),STATUS='OLD')
      CLOSE(UNIT=KDISPO,STATUS='DELETE')
    1 OPEN(UNIT=KDISPO,FILE=NAMGRA(1:LENGRA),STATUS='NEW')
      RETURN
C
