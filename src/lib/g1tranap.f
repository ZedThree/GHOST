...
C
C          ------------------------------------------------
C          ROUTINE NO. (1003)   VERSION (A8.1AP)  14:JUL:97
C          ------------------------------------------------
C
...
C
   16   IF (IXVAL.NE.-17) GO TO 19
C
        IGREY= IGREYS(IYVAL)
        INDEX= 4
        IF (IGREY.NE.0.AND.IGREY.LT.100) GO TO 17
C
        ICODES(2)= IZERO+(IGREY/100)
        GO TO 18
C
   17   ICODES(2)= IPOINT
        ICODES(3)= IZERO+IGREY/10
        ICODES(4)= IZERO+MOD(IGREY,10)
        INDEX= 6
   18   ICODES(INDEX-1)= IBLANK
        ICODES(INDEX)= LETTRG
        CALL G1BUFF(ICODES,INDEX)
        GO TO 100
C
...
C
   19   IF (IXVAL.NE.-18) GO TO 24
        IF (INDSAV.GT.0) GO TO 20
C
        BOUND= .FALSE.
        GO TO 100
C
...
C
   20   IF (IXSAVE(1).EQ.IXSAVE(INDSAV).AND.
     &      IYSAVE(1).EQ.IYSAVE(INDSAV)) GO TO 21
C
        CALL G1DISC(IXSAVE(1),IYSAVE(1))
   21   IF (IHTCHP(IHTYPE,2).NE.0) GO TO 22
C
...
C
        ...
        GO TO 23
C
   22   IF (IHTCHP(IHTYPE,1).LE.0) GO TO 23
C
C          THIS SECTION IS FOR CROSS-HATCHING.
C
        IF (MOD(IHTCHP(IHTYPE,1),2).EQ.1) CALL G1SHAD(1)
        IF (IHTCHP(IHTYPE,1)/2.EQ.1)      CALL G1SHAD(2)
C
   23   BOUND= .FALSE.
        GO TO 100
C
...
C          (IT IS NOT IMPLEMENTED IN THIS VERSION).
C
   24   IF (IXVAL.NE.-19) GO TO 25
        GO TO 100
C
C          IF <IXVAL>= -20, A FILLED AREA BOUNDARY IS BEGUN.
C
   25   IF (IXVAL.NE.-20) GO TO 29
...
C
        ...
        GO TO 100
C
   26   IF (IYVAL.GE.0) GO TO 27
C
        IYVAL= -IYVAL-1
        IF (INDSAV.EQ.0) GO TO 28
C
...
C
   27   IF (INDSAV.GT.0) GO TO 28
C
        INDSAV= 1
        IXSAVE(1)= IXMSAV
        IYSAVE(1)= IYMSAV
   28   IF (INDSAV+3.GE.ISAVSZ) GO TO 100
C
        INDSAV= INDSAV+1
        IXSAVE(INDSAV)= IXVAL
        IYSAVE(INDSAV)= IYVAL
...
C          (IT IS NOT IMPLEMENTED IN THIS VERSION).
C
   34   IF (NUMFUN.NE.-41) GO TO 35
        GO TO 100
C
C          IF <NUMFUN>= -42, HARDWARE LINE TYPE IS SET.
C
   35   IF (NUMFUN.NE.-42) GO TO 39
        IF (LENARG.EQ.0) GO TO 36
C
        ICODES(2)= LETTSB
        CALL G1BUFF(ICODES,2)
   36   INDEX= 1
...
C          (IT IS NOT IMPLEMENTED IN THIS VERSION).
C
   39   IF (NUMFUN.NE.-43) GO TO 40
        GO TO 100
C
C          IF <NUMFUN>= -44, SET HATCHING PARAMETERS.
C
   40   IF (NUMFUN.NE.-44) GO TO 100
C
        IHATPT= 11-LENARG
...
C
   41   IF (INDXB.GT.INITXB) CALL G1DVIO(3,ICODEB,INDXB-1)
C
        INDXB= INITXB
C
...
C
        IF (IXVAL.NE.-81) GO TO 42
        GO TO 43
C
C          IF <IXVAL>= -82, AN ERASE WITHOUT PROMPT IS DONE.
C
   42   IF (IXVAL.NE.-82) GO TO 48
C
   43   ICODES(2)= LETTSP
...
C
   58   DO 300 I= 1,5
          ...
          INDXBT= I
          IVCTOR= IDIVSN
          IF (IDIVSN.EQ.0) GO TO 59
  300   CONTINUE
C
   59   DO 400 I= 1,INDXBT
          IPOS1= INDEX+I
          IPOS2= INDXBT-I+1
...
C
        GO TO (7,9,13,37,38,44,45,46,47,51,52,53,54), IFROM
  100 CONTINUE
C
      RETURN
      END
