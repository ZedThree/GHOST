      SUBROUTINE G0SPLT(SURFAS,ISTRTX,ISTOPX,NPTSX,
     &                  ISTRTY,ISTOPY,NPTSY)
C
C          ------------------------------------------------
C          ROUTINE NO. ( 366)   VERSION (A9.1)    13:JAN:94
C          ------------------------------------------------
C
      PARAMETER (ISZARR= 1200, JSZARR= 600)
      REAL    SURFAS(NPTSX,NPTSY),XPOS(JSZARR),YPOS(JSZARR)
      INTEGER IPTRS(JSZARR)
      LOGICAL ERRON
C
...
C
     ...
     &          (XPOS(2)-XPOS(1))+YPOS(1)
          IVIS2= 0
          IF (SYPOS2.GT.YVAL) IVIS2= ITPBTM
          IF (SYPOS2.LT.YVAL) IVIS2= -ITPBTM
C
          YVAL= (SXPOS3-XPOS(INDEX3))*(YPOS(INDEX3+1)-YPOS(INDEX3))/
     &          (XPOS(INDEX3+1)-XPOS(INDEX3))+YPOS(INDEX3)
          IVIS3= 0
          IF (SYPOS3.GT.YVAL) IVIS3= ITPBTM
          IF (SYPOS3.LT.YVAL) IVIS3= -ITPBTM
C
          YVAL= (SXPOS4-XPOS(LSTSIZ-1))*(YPOS(LSTSIZ)-YPOS(LSTSIZ-1))/
     &          (XPOS(LSTSIZ)-XPOS(LSTSIZ-1))+YPOS(LSTSIZ-1)
          IVIS4= 0
          IF (SYPOS4.GT.YVAL) IVIS4= ITPBTM
          IF (SYPOS4.LT.YVAL) IVIS4= -ITPBTM
C
...
C
     ...
     &          (SXPOS3-SXPOS2)+SYPOS2
          IVIS1= 0
          IF (YPOS(IND).GT.YVAL) IVIS1= ITPBTM
          IF (YPOS(IND).LT.YVAL) IVIS1= -ITPBTM
...
C          CASE 3
C
            IF (ABS(XINT1-XSAV).GT.RMINI.OR.ABS(YINT1-YSAV).GT.RMINI)
     &          CALL POSITN(XINT1,YINT1)
C
            CALL JOIN(XINT2,YINT2)
            XSAV= XINT2
            YSAV= YINT2
...
C
   12     IF (IVISL.EQ.0) CALL G0SDEL(IPTRS(LSTSIZ))
          IF (INDC.EQ.0.AND.ITPBTM.GE.0) THEN
            IF (ABS(SXPOS4-XSAV).GT.RMINI.OR.ABS(SYPOS4-YSAV).GT.RMINI)
     &          CALL POSITN(SXPOS4,SYPOS4)
C
...
C          DRAW THE BASE.
C
   13     IF (INDC.EQ.0.AND.INDR.EQ.0) THEN
            YBAS1= VSCALE*SURMIN+YSHIFT
            CALL POSITN(SXPOS1,SYPOS1)
            CALL JOIN(SXPOS1,YBAS1)
          ENDIF
C
          IF (INDR.EQ.0) THEN
            YBAS2= INDC1*DELTYC+VSCALE*SURMIN+YSHIFT
            CALL POSITN(SXPOS2,SYPOS2)
            CALL JOIN(SXPOS2,YBAS2)
          ENDIF
C
          IF (INDC.EQ.0) THEN
            YBAS4= INDR1*DELTYR+VSCALE*SURMIN+YSHIFT
            CALL POSITN(SXPOS4,SYPOS4)
...
C
      ...
      IF (ITPBTM.EQ.0.AND.TLTANG.LT.0.0) RETURN
      IF (ITPBTM.EQ.0.AND.INDAXE.GT.0) RETURN
      IF (ITPBTM.EQ.1.AND.INDAXE.EQ.0) GO TO 14
      IF (ITPBTM.EQ.1.AND.TLTANG.LT.0.0) GO TO 14
C
C          DRAW BASE LINES.
C
...
C
   ...
   14 IF (ISURIN.NE.0) CALL G0SIND(XEND2,XEND4,YEND2,YEND4)
C
      RETURN
C
  901 NUMERR= 41
