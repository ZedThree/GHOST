...
C
C          ------------------------------------------------
C          ROUTINE NO. (3001)   VERSION (A8.8C)   07:NOV:96
C          ------------------------------------------------
C
...
C
      DO 400 NHATAB= 65,255
        IF (NHFLG0(NHATAB).NE.0) THEN
          IARGS(1)= NHATAB
          CALL G1CLS5(12,-1,IARGS,RARGS)
          RARGS(1)= HTANG0(NHATAB,1)
          RARGS(2)= 1.0
          CALL G1CLS5(5,2*NBYTR,IARGS,RARGS)
          RARGS(1)= HTANG0(NHATAB,2)
          RARGS(2)= 2.0
          CALL G1CLS5(5,2*NBYTR,IARGS,RARGS)
          IARGS(1)= IPITH0(NHATAB,1)
          IARGS(2)= 1
          CALL G1CLS5(6,-2,IARGS,RARGS)
          IARGS(1)= IPITH0(NHATAB,2)
          IARGS(2)= 2
          CALL G1CLS5(6,-2,IARGS,RARGS)
          IARGS(1)= ISHFT0(NHATAB,1)
          IARGS(2)= 1
          CALL G1CLS5(7,-2,IARGS,RARGS)
...
C
C          THIS SECTION SUPPLIES THE GRIDFILE-HANDLER WITH DATA
C          FOR ALL FUNCTIONS OTHER THAN THOSE BUFFERED ABOVE.
C
C          IF GRIDFILE OUTPUT HAS BEEN DISABLED, ONLY A 'GREND'
C          FUNCTION WILL BE SENT ON TO THE GRIDFILE-HANDLER. IN
C          ALL OTHER CASES, ONLY THE PICTURE STATUS IS UPDATED.
C          DEVICE-CHANNEL ENABLE/DISABLE AND BUFFER-CLEARING
C          FUNCTIONS HAVE NO EFFECT ON GRIDFILE OPERATIONS.
C          SUCCESSIVE 'NEW-PICTURE' FUNCTIONS ARE NOT SAVED.
C
   19 IF (ICODE.EQ.34.OR.ICODE.EQ.37) GO TO 24
      IF (.NOT.GFOPEN) GO TO 25
...
C
      IF (ICODE.EQ.35) GO TO 26
      IF (ICODE.NE.33.AND.ICODE.NE.36.AND..NOT.NEWPIC) GO TO 23
      IF (ICODE.EQ.33.AND.LVLPIC.NE.0) GO TO 24
C
...
C
   ...
   20 IARGS(1)= 1
      ...
      IF ((ICODE.EQ.33.OR.ICODE.EQ.36).AND.MTRAN0.GE.2) GO TO 21
C
...
C
   24 IF (ICODE.EQ.97.OR.ICODE.EQ.98) GO TO 25
      CALL G2BUFF(ICODE,LENGTH,IDATA,RDATA)
   25 NEWPIC= PICCOM
      RETURN
C
   26 IF (MTRAN0.GE.2.OR.LVLPIC.NE.1) CALL G2BUFF(35,0,IDATA,RDATA)
