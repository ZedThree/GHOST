      SUBROUTINE SURCL4(SURFAS,ISTRTX,ISTOPX,NPTSX,ISTRTY,ISTOPY,NPTSY)
C
C          ------------------------------------------------
C          ROUTINE NO. ( 341)   VERSION (A9.1)    18:JUL:95
C          ------------------------------------------------
C
C          THIS PLOTS A SURFACE PROJECTION OF THE REGULAR
C          RECTANGULAR GRID OF SPOT HEIGHTS IN SURFAS AND
C          FILLS EACH FACET WITH A COLOUR FROM AN ARRAY.
C
C
...
C
      ...
      ITRAC3= ISTRTY
      ITRAC4= ISTOPY
      IF (IPRINT.EQ.1) CALL G0MESG(189,8)
C
      ILENX= ISTOPX-ISTRTX
      ILENY= ISTOPY-ISTRTY
      XLEN= ILENX
...
C
      ITPBTM= 1
      CALL LINCOL(LINCUP)
      CALL G0SCL4(SURFAS,ISTRTX,ISTOPX,NPTSX,ISTRTY,ISTOPY,NPTSY)
C
      IF (INDBAS.EQ.1) THEN
        ITPBTM= 0
        CALL LINCOL(LINCBS)
