      SUBROUTINE SURFL4(SURFAS,ISTRTX,ISTOPX,NPTSX,ISTRTY,ISTOPY,NPTSY,
     &                  CLEVLS,ISTRTL,ISTOPL,SURFS2,ISTX2,NPTSX2,
     &                  ISTY2,NPTSY2)
C
C          ------------------------------------------------
C          ROUTINE NO. ( 344)   VERSION (A9.1)    18:JUL:95
C          ------------------------------------------------
C
C          THIS PLOTS A SURFACE PROJECTION OF THE REGULAR
C          RECTANGULAR GRID OF SPOT HEIGHTS IN SURFAS AND
C          DRAWS A FILLED CONTOUR MAP OF A SECOND SURFACE
C          ON THE PROJECTION.
C
C
...
C          [SURFS2]  IS THE ARRAY OF SECOND SURFACE HEIGHT VALUES.
C          <ISTX2>   IS THE LOWER X-EXTENT.
C          <ISTY2>   IS THE LOWER Y-EXTENT.
C          <NPTSX2>  IS THE ACTUAL ARRAY X-EXTENT.
C          <NPTSY2>  IS THE ACTUAL ARRAY Y-EXTENT.
C
C
      REAL    SURFAS(NPTSX,NPTSY),CLEVLS(ISTOPL),
     &        SURFS2(NPTSX2,NPTSY2),ARG(2)
      LOGICAL ERRON
C
...
C
      ...
      ITRAC3= ISTRTY
      ITRAC4= ISTOPY
      IF (IPRINT.EQ.1) CALL G0MESG(192,8)
C
      ILENX= ISTOPX-ISTRTX
      ILENY= ISTOPY-ISTRTY
      XLEN= ILENX
      ...
      IF (ISTX2.LT.1.OR.ISTY2.LT.1)           GO TO 901
      IF (ISTX2+ILENX.GT.NPTSX2)              GO TO 901
      IF (ISTY2+ILENY.GT.NPTSY2)              GO TO 901
...
C
      ITPBTM= 1
      CALL LINCOL(LINCUP)
      CALL G0SFL4(SURFAS,ISTRTX,ISTOPX,NPTSX,ISTRTY,ISTOPY,NPTSY,
     &            CLEVLS,ISTRTL,ISTOPL,SURFS2,ISTX2,NPTSX2,
     &            ISTY2,NPTSY2,1)
C
      IF (INDBAS.EQ.1) THEN
