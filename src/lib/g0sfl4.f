      SUBROUTINE G0SFL4(SURFAS,ISTRTX,ISTOPX,NPTSX,ISTRTY,ISTOPY,
     &                  NPTSY,CLEVLS,ISTRTL,ISTOPL,SURFS2,ISTX2,
     &                  NPTSX2,ISTY2,NPTSY2,MODE)
C
C          ------------------------------------------------
C          ROUTINE NO. ( 359)   VERSION (A9.1)    13:JAN:94
C          ------------------------------------------------
C
C
     ...
     &        SYPT(4),SHPT(4),SURFS2(NPTSX2,NPTSY2)
...
C
      ...
      ELSE IF (IQDRNT.EQ.3) THEN
        ISTRTA= ISTOPX
        ISTOPA= ISTRTX
        ISTRTC= ISTX2-ISTRTX+ISTOPX
        INCA= -1
        ISTRTB= ISTOPY
        ISTOPB= ISTRTY
        ISTRTD= ISTY2-ISTRTY+ISTOPY
        SHPOS= MAX(MIN(SURFAS(ISTRTA,ISTRTB),SURMAX),SURMIN)
        INCB= -1
        IDIR= 0
      ELSE
...
C
      DO 200 JJ= ISTRTB,ISTOPB-INCB,INCB
        ...
        IF (IDIR.EQ.0) THEN
          ...
          SHPT(1)= SURFS2(ISTRTC,JJJ)
          SHPT(2)= SURFS2(ISTRTC,JJJ+INCB)
        ELSE
          SHPOS1= MAX(MIN(SURFAS(JJ,ISTRTA),SURMAX),SURMIN)
          SHPOS2= MAX(MIN(SURFAS(JJ+INCB,ISTRTA),SURMAX),SURMIN)
          SHPT(1)= SURFS2(JJJ,ISTRTC)
          SHPT(2)= SURFS2(JJJ+INCB,ISTRTC)
        ENDIF
C
...
C
        DO 200 II= ISTRTA,ISTOPA-INCA,INCA
          ...
          IF (IDIR.EQ.0) THEN
            ...
            SHPT(3)= SURFS2(III+INCA,JJJ+INCB)
            SHPT(4)= SURFS2(III+INCA,JJJ)
          ELSE
            SHPOS3= MAX(MIN(SURFAS(JJ+INCB,II+INCA),SURMAX),SURMIN)
            SHPOS4= MAX(MIN(SURFAS(JJ,II+INCA),SURMAX),SURMIN)
            SHPT(3)= SURFS2(JJJ+INCB,III+INCA)
            SHPT(4)= SURFS2(JJJ,III+INCA)
          ENDIF
C
